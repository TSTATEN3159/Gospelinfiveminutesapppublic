...
// ---------- Per-session level gating (your UI levels) ----------
...
const LEVEL_MAP: Record<LevelKey, string> = {
  beginner: "beginner", // TRIVIA_LEVELS.beginner
  intermediate: "student", // TRIVIA_LEVELS.student
  advanced: "expert", // TRIVIA_LEVELS.expert
};

// ---------- Global progression model (mirrors server TRIVIA_LEVELS) ----------
...
// Matches server calculateLevel(totalCorrectAnswers)
...
// Matches server getNextLevelProgress(totalCorrectAnswers)
function getGlobalProgress(totalCorrect: number): {
  ...
  // Determine where this level “starts”
...
export default function BibleTriviaPage({
  ...
  // --- Level & progression state (per-session) ---
  ...
  // --- Global stats (all-time on this device) ---
  ...
  // Load stored highest unlocked level & global stats on mount
  useEffect(() => {
    ...
    if (storedLevel && (LEVEL_ORDER as string[]).includes(storedLevel)) {
      const storedLevelKey = storedLevel as LevelKey;
      setHighestUnlockedLevel(storedLevelKey);
      setCurrentLevel(storedLevelKey);
    }
  ...
  // Update & persist global stats (called after each answered question)
  ...
  const fetchQuestionsForLevel = useCallback(
    async (level: LevelKey) => {
      setLoading(true);
      setError(null);
      setJustUnlockedLevel(null); // reset “you unlocked” banner
  ...
  const handleSelectOption = (index: number) => {
    // Prevent changing answer after choosing
  ...
  const handleNextQuestion = () => {
    ...
    // 🔁 Update global stats after each answered question
    ...
    const reachedEnd =
      nextIndex >= QUESTIONS_PER_GAME || nextIndex >= questions.length;
  ...
  useEffect(() => {
    ...
    // Unlock only if this level was at/above previous highest
  ...
  const isLastQuestion =
    currentIndex >= QUESTIONS_PER_GAME - 1 ||
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              <div className="flex justify-between text-sm">
                <span>
                  Correct answers:{" "}
                  <strong>{globalStats.totalCorrect}</strong>
                </span>
                <span>
                  Questions answered:{" "}
  ...
  if (gameState === "finished") {
    const nextLevel =
      hasNextLevel ? LEVEL_ORDER[currentLevelIndex + 1] : null;
    const justUnlockedIsNext = justUnlockedLevel === nextLevel;
    const canGoToNextLevel =
      !!nextLevel && highestLevelIndex >= LEVEL_ORDER.indexOf(nextLevel);
          ...
          {passed && (
            ...
            >
              <CardContent className="p-6 text-center">
                ...
                {justUnlockedLevel && nextLevel === justUnlockedLevel && (
                ...
                {canGoToNextLevel ? (
                  justUnlockedIsNext ? (
                    <>
                      ...
                      Try {LEVEL_CONFIG[nextLevel!].name} Level
                    </>
                  ) : (
                    <>Go to {LEVEL_CONFIG[nextLevel!].name} Level</>
                  )
                ) : (
                  <>
